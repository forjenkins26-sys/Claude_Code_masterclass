...
if (!url) {
  console.error('Usage: node fetch-local-page.js <url>');
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    const analysis = await page.evaluate(() => {
      ...
      const buttons = Array.from(document.querySelectorAll('button, [role="button"], input[type="submit"], input[type="button"], a.btn, a[class*="button"]')).map(el => {
        const hasClickHandler = el.onclick !== null || el.getAttribute('onclick') !== null;
        const listeners = typeof getEventListeners !== 'undefined'
          ? Object.keys(getEventListeners(el))
          : [];
        return {
          tag: el.tagName.toLowerCase(),
      ...
      // Visible text content (headings + paragraphs)
      const textContent = Array.from(document.querySelectorAll('h1,h2,h3,h4,p,label,span.tagline,.sub'))
    ...
    // Playwright-suggested locators for key elements
    const suggestions = [];
