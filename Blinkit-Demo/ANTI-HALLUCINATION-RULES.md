> **Reference copy.** The authoritative version lives at the workspace root:
> `c:\ClaudeCodeMasterclass\ANTI-HALLUCINATION-RULES.md`
> These rules are already **inlined inside each skill** — no skill reads this file by path.
> It is included here so the ruleset can be read alongside the project. If it disagrees with
> the root copy, the root copy wins.
