# RAG Explorer — Exact Rebuild Spec

**Reproduces:** https://frontend-six-alpha-35.vercel.app/ (Vercel) + https://rag-explorer-api.onrender.com (Render)
**Verified 2026-08-28** by reading `app/backend/*.py`, `app/frontend/src/**`, `render.yaml`, `Dockerfile`, the compiled `dist/assets/*.js`, and a live `GET /api/status` response.

Feed the **Prompt** section below to rebuild the deployed system byte-for-byte in behaviour.

---

## Live state at time of verification

```json
{"embedding_model":"nomic-embed-text-v1.5","llm_model":"llama-3.3-70b-versatile",
 "vector_db":"ChromaDB (local, persistent)",
 "pdf_folder":"/opt/render/project/src/backend/data/pdf",
 "total_chunks":0,"embedding_dims":0,"sample_embedding":[],"sources":{}}
```

That is the **running service**, captured before the fixes below. It disagreed with local:

| | Local `.env` | `render.yaml` (repo) | **Running service** |
|---|---|---|---|
| Groq model | `openai/gpt-oss-120b` | ~~`llama-3.3-70b-versatile`~~ → **fixed 2026-08-28** | **still `llama-3.3-70b-versatile`** |
| Chunks stored | populated | — | **0 — empty store** |

**The running backend is on a model Groq has decommissioned.** It has not errored only because the
store is empty — `/api/query` returns `"No chunks retrieved"` and exits *before* calling Groq.

The repo is now corrected. **The live service still needs one manual dashboard change** — see
[⚠ Live deployment](#-live-deployment--one-manual-step-still-outstanding) at the end. This spec
builds the system as it *should* be, with that gap flagged where it lives.

Render free tier cold-sleeps: first request after idle took **5.1s**. A 25s client timeout returned `000`. Allow 60–90s on a cold hit.

---

## Which prompt file to use

| File | What it is | Rebuilds the live site? |
|---|---|---|
| `REQUIREMENTS.md` | Original ask, Jul 20 15:56 | No — pre-iteration |
| `Prompt.md` | As-built spec, Jul 20 19:57 | **No — wrong embedding architecture** |
| `SimplePrompt.md` | Short retelling, Aug 28 | No — no deployment at all |
| **this file** | Verified from source + live API | **Yes** |

`Prompt.md`'s two fatal instructions: it says embeddings run locally via `sentence-transformers` with "no Nomic API key" (the deployed app uses Nomic's **hosted** API and requires a key — `torch` appears **0 times** in `requirements.txt`), and it recommends Hugging Face Spaces (you shipped on Render). A build following it will OOM on a free tier. Keep it as the historical local-build record.

---

# The Prompt

> Build and deploy a full-stack **RAG Explorer** — a React UI that makes every stage of a
> Retrieval-Augmented Generation pipeline visible (ingest → chunk → embed → store → retrieve →
> answer), so a viewer watches each step happen rather than just receiving a final answer.
>
> Frontend deploys to **Vercel** as a static build; backend deploys to **Render** as a Python web
> service. They are separate deployments wired by one build-time env var.
>
...
> - Chunk each section with a fixed sliding window — `CHUNK_SIZE` chars, advancing
...
> - The UI lists each ingested source with a hover-revealed `✕` button calling that route, then
>   clears the current query result — it may cite chunks that no longer exist.
...
> Five wrappers: `getStatus`, `getChunks`, `runQuery`, `reingest`, `uploadPdf`. `uploadPdf` sends
> `FormData` and surfaces `body.detail` as the error message so the 400 reaches the user.
>
...
> Numbered circle + label + sub, `→` between. Three states from `activeStep`:
> **done** (`>` index) `border-emerald-800 bg-emerald-950/40` · **active** (`===`)
> `border-emerald-500/60 bg-emerald-500/10` · **idle** `border-neutral-800 bg-neutral-900/40`
> with a `bg-neutral-800 text-neutral-500` circle. Horizontal, `overflow-x-auto`.
>
...
> clickable, that submit immediately:
> *"What is the goal of this PRD?"* · *"Who are the target users?"* ·
> *"What are the key features described?"* · *"What are the success metrics?"*
...
> services:
>   - type: web
>     name: rag-explorer-api
>     runtime: python
>     plan: free
>     rootDir: RAG/Basic_Rag/app/backend
...
>
> **Frontend → Vercel.** Static Vite build, no `vercel.json` needed. Set `VITE_API_URL` to the
> Render URL **as a build-time env var** — it is inlined into the bundle, so changing it requires a
> rebuild, not just a redeploy.
>
> **After the backend is up,** set `CORS_ORIGINS` to the real Vercel origin and drop the `*`.
>
> A `Dockerfile` also exists for Hugging Face Spaces (non-root `appuser`, `EXPOSE 7860`). Render
> uses the native Python runtime, not this file.
>
> ## Non-negotiables
>
...
## Environment gotchas — fix proactively
...
## ⚠ Live deployment — one manual step still outstanding

The repo is fixed. **The running service is not.**

`render.yaml` and `.env.example` were corrected to `openai/gpt-oss-120b` on 2026-08-28. But
`render.yaml` only seeds env vars when a service is **first provisioned** — for an already-running
service the **dashboard value wins**, and the dashboard still holds the retired
`llama-3.3-70b-versatile`. A redeploy alone will not change it.

The failure is **latent, not visible**: the deployed store is empty (`total_chunks: 0`), so
`/api/query` returns `"No chunks retrieved"` and exits *before* it ever calls Groq. It looks healthy.

**Ingest one document into the live backend and the next query returns `404 model_not_found`.**

### Do this before any client demo

1. Render dashboard → service `rag-explorer-api` → **Environment**
2. Set `GROQ_MODEL` = `openai/gpt-oss-120b` → **Save** (triggers a redeploy)
3. Verify — the value must come back as `gpt-oss`, not `llama`:

   ```bash
   curl -s --max-time 90 https://rag-explorer-api.onrender.com/api/status
   ```

4. While you are there, confirm `CORS_ORIGINS` is the real Vercel origin
   (`https://frontend-six-alpha-35.vercel.app`) and not the `*` wildcard fallback.
5. Upload a document through the live UI, then ask one question — that is the only check that
   actually exercises the Groq call end to end.

Until step 5 passes, treat the live LLM path as **unverified**.
...
**Sample document** — `Product Requirements Document_(PRD)_VWO.com.pdf`, in this folder.
Try *"What is SmartStats?"* or *"What are the functional requirements?"*

---

## Connection to the QA framework

`/test-case-creation` **Mode C** calls this backend at `localhost:8000` to generate test cases from
a spec document instead of a Jira Epic. Two binding rules:

- Assertions quote the raw `chunk.content` — **never** the synthesised `answer`. A paraphrase is
  not traceable to a requirement.
- Similarity below **0.5** is reported as a requirement gap, never forced into an assertion.

Mode C health-checks first:
`curl -s -o /dev/null -w "%{http_code}" --max-time 3 http://localhost:8000/docs` — 200 or stop.
