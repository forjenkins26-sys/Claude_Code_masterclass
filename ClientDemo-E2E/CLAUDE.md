...
| **App** | MediCare Pharmacy — login page |
...
The app is public, so `/explore` uses **live Playwright MCP**, not `scripts/fetch-local-page.js`. That script is present only in case a localhost URL is used instead.

## Demo Flow (the 4-step story)

| # | Command | What the client sees | Takes |
|---|---|---|---|
| 1 | `/explore <URL>` | Live browser opens, DOM read, Page Object generated from real elements | ~2 min |
| 2 | `/test-case-creation <EPIC>` | Requirements → test cases created in Jira, edge-case matrix walked per control | ~4 min |
| 3 | `/test-case-execution <EPIC>` | Headed run. **Failures classified before any fix.** Real bugs filed, test issues auto-fixed | ~5 min |
| 4 | `/test-closure <EPIC>` | AC→test traceability, counted coverage, Go / No-Go verdict | ~2 min |

Optional 5th: `/requirement-drift <EPIC>` — change an AC in Jira mid-demo, then show it detecting the drift and scoping the re-test.
