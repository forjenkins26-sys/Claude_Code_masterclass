...
SCAN_RE='CLAUDE\.md|ANTI-HALLUCINATION|AUTO-FIX|QA-SKILLS|LOCAL-GUARD|HOW-TO-USE-GUARD|INSTALL\.md|knowledge-base/|^skills/|agent-factory|rule-engine|framework-rule-engine|settings-hooks|package-scripts|setup-local-guard|verify-locators-|progress\.md|rca-log\.md|findings\.md|task_plan\.md|\.env|\.vercel/'

# tracked paths to untrack if found (folders + root files; --cached keeps them on disk)
UNTRACK_PATHS='CLAUDE.md ANTI-HALLUCINATION-RULES.md AUTO-FIX-PROTOCOL.md QA-SKILLS-CHEATSHEET.md LOCAL-GUARD-SETUP.md HOW-TO-USE-GUARD.md INSTALL.md skills agent-factory agent-factory-cli knowledge-base rules scripts/rule-engine.js settings-hooks.json package-scripts.json setup-local-guard.sh'
