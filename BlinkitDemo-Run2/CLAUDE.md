...
**SCRUM-694 verified fresh: 0 child issues.** 15 ACs, login page only.

**Do not create another Epic.** Several exist for this same page — a duplicate splits test cases across Epics and breaks the traceability the demo is meant to show.

Blinkit login Epics that are NOT this one:

| Key | Why not |
|---|---|
| SCRUM-672 | **consumed** — 21 child test cases |
| SCRUM-653 | already fully run — 18 children, closure report exists |
...
| 2 | `/test-case-creation SCRUM-694` | Requirements → test cases in Jira, edge-case matrix walked per control | ~4 min |
| 3 | `/test-case-execution SCRUM-694` | Headed run. **Failures classified before any fix.** Real bugs filed, test issues auto-fixed | ~5 min |
| 4 | `/test-closure SCRUM-694` | AC→test traceability, counted coverage, Go / No-Go verdict | ~2 min |
...
Wired in `playwright.config.ts` — per-run `allure-results/{RUN_ID}/` (AH Rule 31), which is what makes the cross-run trend graph work.
...
`/test-closure` Step 4B attaches this report when `allure-results/` exists. Allure is **evidence, not the oracle** — `progress.md` remains the source of truth for pass/fail.
...
**Narrowing `fields` does not reliably fix it** — the bulk is the Atlassian MCP response envelope (`context` object, account/cloud ids, client metadata), which `fields` does not trim.

**Working path when `getJiraIssue` / `searchJiraIssuesUsingJql` archive** — `mcp__atlassian__fetch` does not carry that envelope:
...
Step 1 must return few results — quote the exact title to narrow it. `cloudId` for this site: `e74af77d-c1bf-4809-aa7e-20b6020a077b`.

**Never trust an archived write.** A `createJiraIssue` whose response archived may have succeeded *or* failed — always re-query by search/JQL to confirm the issue exists before relying on its key. Both happened here: on 2026-08-24 one Epic reported as created never existed, and SCRUM-694's own create response archived but had in fact succeeded.
...
knowledge-base/SCRUM/
  business-rules.md    ← bug oracle (BR-xx). Seed from the SCRUM-694 ACs before Step 3
...
**Seed `business-rules.md` from the SCRUM-694 ACs during Step 2.** Without it, Step 3 loses Confirmed/Suspected bug tiering and dedup — and it fails *silently*.

**Dedup warning:** earlier Blinkit Epics (SCRUM-121, SCRUM-386, SCRUM-403, SCRUM-603, SCRUM-626, SCRUM-653, SCRUM-672) already carry filed defects for this same app. If a defect here matches one of those, reference it rather than filing a duplicate (AH Rule 21).
...
All live globally at `~/.claude/skills/` and travel to every project — **never copied per project**. Currently at the 2026-08-18 baseline.
