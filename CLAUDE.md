...
src/
  pages/
    ...
    BlinkitProductsPage.ts    ← Blinkit products POM + catalogue-API state stubs (page.route)
...
tests/
  ui/
    ...
    registration.spec.ts      ← REG-001–REG-034 (SCRUM-142, registration-demo.html)
    blinkit-products.spec.ts  ← BL-020–BL-024 (SCRUM-121) — catalogue API states
...
| SCRUM-86 | Facebook Registration | superseded — `registration.spec.ts` now targets SCRUM-142 | Done |
| SCRUM-121 | Blinkit Login | BL-001–BL-019 (18/19; BL-010 blocked) | Active |
| ~~SCRUM-142~~ | Registration Demo Page — **Epic key does not exist in Jira** (verified 2026-09-15, 404; no registration Epic exists — all 9 Epics are Blinkit). Test cases DO exist as standalone Stories (REG-017=SCRUM-159, REG-018=SCRUM-160, REG-011=SCRUM-112). Spec is REG-001–REG-034 + REG-018b (28/35; all 7 failures known defects: BUG-A ×3, BUG-B ×2, BUG-C=SCRUM-795, REG-032 blocked by BUG-A) | Needs Epic |
| SCRUM-121 | Blinkit Products catalogue | BL-020–BL-024 (5/5) — API load/error/empty/retry states | Active |
...
- **SCRUM-795 (BUG-C): age gate ignores day-of-month** — `registration-demo.html` `checkAge()` returns `age > 18 || (age === 18 && m >= 0)`, comparing years and months only, so anyone born earlier in the current month passes the gate **before their 18th birthday**. Verified: a DOB 3 days short of 18 is accepted. Violates **BR-13**. Caught by REG-018b; Blocks SCRUM-160. Distinct from the seeded BUG-A/BUG-B — found 2026-09-15 while fixing REG-018's stale date
...
- `ANTI-HALLUCINATION-RULES.md` — 33 QA verification rules (Rule 33: a gap in the requirement is a finding to report, not a blank to fill — never fabricate an AC/field/threshold, never rewrite the ticket for the author, every finding traceable to a line or its absence, a readiness verdict is advisory; promoted from `test-case-creation` Step 1C so it applies to every skill that reads a requirement; Rule 32: test failing ≠ app broken — re-derive the expected value before assigning REAL_BUG, inverse of Rule 19; Rule 31: harvest evidence before any second Playwright run — `test-results/` is wiped on every invocation, even a zero-match one; Rule 30: recalled memory is a claim not a fact — re-verify before use, closes KB feedback-loop risk; Rule 29: edge-case coverage matrix; Rule 27: URL scope / nav-tests; Rule 26: stay in command scope; Rule 25: KB bug-oracle + Confirmed/Suspected tiers; Rule 23: 4-category failure taxonomy)
...
knowledge-base/
  ...
  SCRUM/                ← active project — seeded from real ACs + bugs
    ...
    run-history.jsonl   ← append-only run store (15 runs seeded from progress.md) — /regression-baseline
    run-baseline.json   ← baseline pointer + build identity + previous (never overwritten)
```

**Regression flow:** `/test-case-execution` Step 0B resolves a composite `build_id`
and records `**Build:**` → `/regression-baseline` parses execution blocks into
`run-history.jsonl` → `/test-closure` Step 3B diffs this run against the baseline
and separates REGRESSION (build got worse) from KNOWN (pre-existing). A changed or
unresolvable build EXPIRES prior results — `UNKNOWN` is never `SAME`.
...
| 2026-08-22 | `app-patterns.json` added as 5th KB file — app-specific pattern memory (field constraints + exact error text, UI conventions, flow end-states, locator strategies, cross-page inconsistencies). Written by `/explore` Step 7B (propose-only, never auto-writes), read by `test-case-creation` Step 3A as an **app-specific layer on top of** the generic control matrix. Three hard limits: hint-not-oracle (Epic AC still owns expected results, AH Rule 19) · re-verify before use, live DOM wins on mismatch (AH Rule 30) · never invent a pattern to fill a gap. Gap it closes: verified real pain — mobile field tested in 3 specs, email in 3, each re-deriving the same edge cases. Seeded from real DOM (`#mobile` maxlength=10, error 'Enter valid 10-digit mobile number'); first run already surfaced inconsistency IN-01 (error-id suffix 'Err' vs 'Error' across pages). Idea from a LinkedIn Reference-JSON architecture, adopted for our own observed pain not feature parity (karpathy Guideline 5); their un-gated memory design deliberately NOT copied | `knowledge-base/SCRUM/app-patterns.json`, `knowledge-base/_TEMPLATE/app-patterns.json`, `~/.claude/skills/explore/SKILL.md`, `~/.claude/skills/test-case-creation/SKILL.md` |
| 2026-08-22 | `/requirement-drift` skill added + build tracking wired into `test-case-execution` Step 0B — closes two gaps found in review: (1) **redeployment blindness** — nothing recorded WHICH build a result belonged to, so a pass on an old build was silently treated as valid after a redeploy; now `**Build:**` is mandatory in the progress.md log and a changed build EXPIRES all prior results (missing build = expired, never 'same'). (2) **AC drift blindness** — `BR-xx` rules cite their source (`SCRUM-255 AC line 8`) but nothing re-compared them against current Jira; now drift reports CHANGED/REMOVED/NEW per rule + maps each to the tests needing re-run. New ACs with no rule surface as coverage gaps. Never auto-edits `business-rules.md` (AH Rule 30 — a wrong oracle entry corrupts every future run). Verified: all 10 sourced BR rules parse to their AC line on real KB data | `~/.claude/skills/requirement-drift/SKILL.md`, `~/.claude/skills/test-case-execution/SKILL.md` |
...
| 2026-09-14 | CLAUDE.md pointer scan (idea from charliehills "graph engineering" article; MAP.md system deliberately NOT adopted — 2% rot doesn't justify a 3rd memory layer alongside BLAST + knowledge-base, karpathy Guideline 5). Fixed 1 broken pointer (`scripts/testplan/` → lives at `output/testplan/` + `src/testplan/`; `new-demo-project.sh` was the real undocumented file) + added 8 tracked-but-invisible projects. Scan wired into `update-claude-md` as **Step 8B** (was prose "no phantom references", never executed — that's how the stale path survived): Part 1 tests claimed paths, Part 2 finds unlisted dirs, triage table prevents subproject-relative false positives. Counted, never estimated: 1 break / 46 paths, 8 reverse orphans, 10 unlisted dirs confirmed gitignored-on-purpose | `CLAUDE.md`, `~/.claude/skills/update-claude-md/SKILL.md` |
| 2026-09-14 | Repo hygiene — untracked 335 files (260 allure reporter output 64MB + 75 root scratch `.png`/`.yml` from 2026-08-31 langflow session); `.gitignore` rules added for `allure-results/`, `allure-report/`, `test-results/`, `playwright-report/`, root `/*.png` + `/*.yml`. Files kept on disk, removed from tracking only. **Security item — RESOLVED 2026-09-14, no leak, no action needed** (see next row) | `.gitignore`, `CLAUDE.md` |
| 2026-09-14 | **`/regression-baseline` skill added** (gap found reverse-engineering a public "QA Orchestrator": `set-baseline` / `compare/{a}/{b}` / `regression` were the one genuinely missing capability — adopted for real pain, karpathy Guideline 5; its SaaS shell, auto-file-every-failure bug agent and mechanical scoring deliberately NOT copied, we already have the KB oracle + LLM-as-Judge). Two scripts: `build-identity.js` (composite `sha256(etag\|last-modified\|content-length)`, 3 states, `UNKNOWN`≠`SAME`) + `regression-baseline.js` (progress.md parser → append-only `run-history.jsonl`, baseline pointer, REGRESSION/FIXED/KNOWN/NEW diff, exit 1 gates CI). **Composite, not ETag — empirically forced:** ETag `6fd46ec…830` was byte-identical on 2026-08-23/08-24/09-14 while Last-Modified advanced to Sep 14, so an ETag-only check reports "same build" forever and keeps stale PASSes valid. Verified on real data: 15/15 execution blocks parsed, itemised BLOCKED matched stated `**Results:**` **14/14 runs**; 3 parser defects found+fixed during verification (inner `## Session Score:` h2 truncating a block → 4 blocked tests silently dropped on SCRUM-694 · multi-test bullets `ML-006 + ML-007` recording one id · `BR-xx`/`AC-x` refs miscounted as test ids, inflating SCRUM-603 4→9) | `qa-ai-stack/skills/regression-baseline/`, `knowledge-base/SCRUM/run-history.jsonl`, `run-baseline.json` |
| 2026-09-14 | **TypeScript critic gate** added to `/spec-quality` (`ts-critic.js`) — runs the `tsc --noEmit` the frameworks already define and maps errors to spec files, closing the hole where a non-compiling spec fails at runtime and gets misread as an app bug (AH Rule 32). **Self-promoting ramp, never ask-the-user** (per the 2026-09-10 SCRUM-794 finding: machine-checkable gates missed 0/58 runs, ask-the-user gates 5/58): advisory runs 1–3 → enforcing run 4+. Verified: injected `const n: number = "string"` caught as TS2322+TS2345 and attributed to the spec file; ramp confirmed exit 0,0,0,1,1; probe removed, framework green. One defect found+fixed: Windows CRLF broke the diagnostic regex so real type errors reported as "tsc did not run" | `qa-ai-stack/skills/spec-quality/scripts/ts-critic.js`, `SKILL.md`, `.gitignore` |
| 2026-09-14 | Wiring pass (advisory, non-blocking — no new hard stops; Step 0A remains the only one). `test-case-execution` **Step 0B added** (resolve composite build identity, record mandatory `**Build:**`, expire prior results when CHANGED/UNKNOWN) · `requirement-drift` **Step 4 upgraded** to the composite resolver with the never-compare-a-bare-ETag rule · `test-closure` **Step 3B added** (counted regression check vs baseline, skips silently with no history) + Step 4 NO-GO now triggers on any REGRESSION + `## Regression Note` upgraded to a counted table. **Correction to the 2026-08-22 log row:** it claimed build tracking was wired into `test-case-execution` Step 0B — verified false, the skill had no Step 0B and no build instruction; the practice lived only in `requirement-drift` Step 4 and in `progress.md` convention. Now actually wired. Verified by running each documented command as written (build-identity RESOLVED `f1903d7f64072ec8`; closure regression diff exit 1 on NEW_FAILURE, both text and --json) | `~/.claude/skills/{test-case-execution,requirement-drift,test-closure}/SKILL.md`, `qa-ai-stack/skills/*` |
| 2026-09-15 | **Blinkit products catalogue moved to a real network fetch** — `PRODUCTS` const → `fetch('api/products.json')` (16 products extracted byte-identical) with loading/error/retry states + a `productsLoaded` guard so filters can't clobber them. Reason: reviewing PramodDutta/skillmasterclass surfaced route-mocking as a candidate skill; a grep disproof showed **all 4 demo apps made zero network calls**, so `page.route` had nothing to intercept — the gap was the missing network layer, not a missing skill (karpathy Guideline 5: build the pain before the tool). New `BlinkitProductsPage.ts` POM exposes `stubProductsFailure/Empty/FailingOnce/abort`; BL-020–BL-024 cover happy-path (asserts exactly 1 real request), 500, empty `[]`, aborted connection, and retry recovery. **Regression-checked by stashing every change and re-running**: blinkit-login 15/4 → 15/4, order-details 13/1 → 13/1, registration 27/7 → 27/7, typecheck clean. Net +5 tests, 0 regressions. Adopted 0 of 33 skills from the source repo | `blinkit-products.html`, `api/products.json`, `Playwright Automation Framework/src/pages/BlinkitProductsPage.ts`, `tests/ui/blinkit-products.spec.ts`, `src/fixtures/test-fixtures.ts` |
| 2026-09-15 | **BUG-C filed as SCRUM-795** (High, labels `registration`/`age-gate`/`validation`/`BUG-C`), Blocks SCRUM-160 (REG-018). Dedup ran first per AH Rule 21: KB `known-defects.md` had no age entry, and JQL over open Bugs matching age/birth/DOB/registration/gate returned 6 unrelated tickets — the 3 age-matching issues (SCRUM-159/160/112) are **test cases, not bugs**. KB updated: new **BR-13** (eligibility requires the 18th birthday to have passed, day precision) + defect row + violation cross-ref. **Also found: Epic SCRUM-142 does not exist in Jira** — CLAUDE.md listed it as Active but the key 404s and no registration Epic exists at all (all 9 Epics are Blinkit); the REG test cases live as standalone Stories. Bug filed parentless rather than inventing a parent; Epic table row marked "Needs Epic" | `CLAUDE.md`, `knowledge-base/SCRUM/{known-defects,business-rules}.md`, Jira SCRUM-795 |
| 2026-09-15 | **REG-017/REG-018 stale dates fixed + BUG-C found.** Both age-gate tests hardcoded DOBs written on 2026-06-12; REG-018's `2008-07-12` was 17y11m then but 18y2m by 2026-09-15, so the test failed against a correctly-behaving app (AH Rule 32). Added `RegistrationPage.dobForAge(age, monthsShort)` + `dobTurning18InDays(days)` — both derive from today, so they cannot drift. **Verified across 172 date positions spanning 2026–2029** (month-ends, leap years) before running the suite; the sweep caught two defects in my own first draft: a `.replace(/-\d{2}$/,'-28')` hack that broke on the 28th–31st, and an inverted `monthsShort` sign that produced a 16-year-old instead of 17. New **REG-018b** documents **BUG-C**: `checkAge()` compares years and months only, so a user 3 days short of 18 is accepted — a real defect, separate from the seeded BUG-A/BUG-B. Registration now 28/35, and every remaining failure is a known defect rather than a stale test | `tests/ui/registration.spec.ts`, `src/pages/RegistrationPage.ts`, `CLAUDE.md` |
| 2026-09-15 | **Stale Epic rows corrected** (found while regression-checking the row above — the numbers in CLAUDE.md did not match a real run). SCRUM-86 claimed "REG-001–REG-019, 19/19 passing, Done" but `registration.spec.ts` actually targets **SCRUM-142 / registration-demo.html** with **34 tests, 27 pass / 7 fail**. Of the 7: 5 are intentional seeded defects (BUG-A email `@`-only check, BUG-B password), **REG-032** is blocked by BUG-A (XSS string passes the weak email check, so `emailError` never shows), and **REG-018 is a stale-date test issue, not an app bug** — DOB `2008-07-12` was 17y11m when written on 2026-06-12 but is 18y2m today, so the age gate correctly accepts it (AH Rule 32: re-derive the expected value before calling REAL_BUG). REG-018 needs a relative DOB, not a hardcoded one | `CLAUDE.md` |
| 2026-09-15 | **AH Rule 33 added — "a gap in the requirement is a finding to report, not a blank to fill"** (promoted to global scope). Found by comparing a screenshotted guardrail block against the rules file: the 6 `STRICT RULES (MANDATORY)` matched verbatim, but the 5 Guardrails did not exist in `ANTI-HALLUCINATION-RULES.md` at all — `grep` for "guardrail", "never fabricate", "blank to fill", "READY verdict" returned **zero matches**. They lived only in `test-case-creation` Step 1C + `requirement-gap-checklist.md`, so every other requirement-reading skill (`test-plan-create-skill`, `epic-create`, `bug-triage`, `test-closure`) faced the same pressure with no rule to cite. Rule 33 covers all five clauses: never fabricate an AC/field/threshold · don't rewrite the ticket for the author · every finding traceable to a line **or its explicit absence** · **a readiness verdict is advisory** (author + QA lead own the call — this clause existed nowhere in the repo and was the real gap; an advisory label that hardens into a gate is its own failure mode) · a ❌ answered by a `BR-xx` is not a gap. **Count verified by parsing, not asserted:** 16 numbered rules (1–16) + 17 `## Rule NN` headings (17–33) = 33 distinct, no gaps, no duplicates. Synced 32→33 across **7 tracked references** in 6 files (`ANTI-HALLUCINATION-RULES.md` ×2, both `CLAUDE.md`, both `QA-SKILLS-CHEATSHEET.md`, `qa-ai-stack/INSTALL.md` ×2 lines); `qa-ai-stack` copy confirmed byte-identical to the committed root before syncing. 5 untracked demo copies (ClientDemo26/DemoFinal/DocDemo/MediCare/QuickDemo) deliberately left at 32 — same precedent as the 2026-08-18 sync | `ANTI-HALLUCINATION-RULES.md`, `qa-ai-stack/*`, `CLAUDE.md`, `QA-SKILLS-CHEATSHEET.md`, `~/.claude/skills/test-case-creation/SKILL.md` |
| 2026-09-14 | Scratch-file leak investigation **CLOSED — no key exposed, no rotation, no `git filter-repo` needed.** The 2026-09-14 hygiene row flagged `.png` pixels as unverified; pixels have now been read. Confirmed: PNGs ARE in pushed history (added `755ae19`, only untracked by `1e1b19f` — untracking does not remove from history), repo is PUBLIC. Verified by extracting each file from history and viewing it: `pinecone-cred-modal.png` → API Key field **empty** (shot taken before typing); `pinecone-config.png` → "No credentials yet"; `gemini-config-open.png` → credential shown **by name only** ("Google Gemini(PaLM) Api account 2"), no value; `gemini-node-config.png` + `gemini-panel2.png` → canvas only, no credential UI. All 4 pinecone `.yml` re-scanned in history: zero `pcsk_`/`sk-`/`AIza`/`ghp_`/`xoxb`/`gsk_`/`api_key`/`secret`/`token`. **Mechanism (why this generalises to the other ~70 root scratch files):** n8n never renders a saved credential's value back into the DOM — it shows the credential's *name* in a dropdown, so an n8n screenshot structurally cannot carry a key. Read-only investigation: nothing committed, pushed, edited, or printed | `CLAUDE.md` |
