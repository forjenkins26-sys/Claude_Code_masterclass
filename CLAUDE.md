...
| SCRUM-142 | Registration Demo Page | REG-001–REG-034 + REG-018b (28/35; all 7 failures are known defects: BUG-A ×3, BUG-B ×2, BUG-C ×1, REG-032 blocked by BUG-A) | Active |
...
- **BUG-C (unfiled): age gate ignores day-of-month** — `registration-demo.html` `checkAge()` returns `age > 18 || (age === 18 && m >= 0)`, comparing years and months only, so anyone born earlier in the current month passes the gate **before their 18th birthday**. Verified: a DOB 3 days short of 18 is accepted. Caught by REG-018b. Distinct from the seeded BUG-A/BUG-B — found 2026-09-15 while fixing REG-018's stale date
