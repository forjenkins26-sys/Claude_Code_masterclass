...
| 2026-09-14 | Repo hygiene — untracked 335 files (260 allure reporter output 64MB + 75 root scratch `.png`/`.yml` from 2026-08-31 langflow session); `.gitignore` rules added for `allure-results/`, `allure-report/`, `test-results/`, `playwright-report/`, root `/*.png` + `/*.yml`. Files kept on disk, removed from tracking only. **Open security item:** repo is PUBLIC and `755ae19` pushed those scratch files; all 31 root `.yml` verified clean (zero `pcsk_`/`sk-`/`AIza`/`ghp_`/`xoxb`/`gsk_` matches) but `.png` pixels unverified — `pinecone-cred-modal.png` sits next to `/before-save.yml`, which `.gitignore` flags as having captured a live Pinecone key. Untracking does NOT remove from history; rotate key + `git filter-repo` if a key is visible | `.gitignore`, `CLAUDE.md` |
